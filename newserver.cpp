...
int search_for_peer(char *file_name)
{
	...
	for(long int i=0;i<BUF_MAX;i++)
	{
		if(clients[i])
		{
			...
			found=1;
			downloads[share_port].push_back(string(clients[i]->filename,strlen(clients[i]->filename)));
			break;
		}
	}
	if(found==0)
...
{
	cout<<it->first<<" "<<":";
	for(auto l=it->second.begin();l!=it->second.end();it++)
	{
		cout<<" "<<*l<<" ";
	}
}
